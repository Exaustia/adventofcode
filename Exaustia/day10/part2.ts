...
(async () => {
  ...
  // create 6 arrays of 40 elements of XSet

  const XSetArray = Array.from(XSet.values());

  const allMyArrays = XSetArray.map((x, i) => {
    if (i % 40 === 0) {
      return XSetArray.slice(i, i + 40);
    }
  });

  const xArray: string[][] = [];

  allMyArrays.forEach((element: any, i) => {
    xArray[i] = [];
    element.forEach((el: number, index: number) => {
      if (el + 1 === index || el - 1 === index || el === index) {
        xArray[i].push("#");
      } else {
        xArray[i].push(".");
      }
